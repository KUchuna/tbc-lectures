...
export default function ServiceCard(props: ServiceCard) {
    ...
    return (
        <div className={`card-container ${props.contStyle} dark:bg-slate-600`}>
            ...
            {props.servicepage ? <button className='text-white flex mt-auto dark:text-white bg-service-card-orange px-3 py-2 rounded-xl hover:bg-service-card-hover-orange transition-colors duration-300' onClick={() => props.id !== undefined && handleCartItem(props.id)}>Add to cart</button> :<span className='text-service-card-orange flex mt-auto dark:text-orange-700'>View full information <Image className='ml-6' src={servicearrow} alt="" /></span>}
        </div>
    )
}